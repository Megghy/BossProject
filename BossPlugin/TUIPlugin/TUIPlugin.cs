    ...
    public class TUIPlugin : TerrariaPlugin
    {
        ...
        public override void Initialize()
        {
            try
            {
                ...
                BossFramework.BNet.PacketHandlers.DestroyProjectileHandler.Get += OnProjDestroy;
                BossFramework.BNet.PacketHandlers.SyncProjectileHandler.Get += OnNewProjectile;
        ...
        private void OnGamePostInitialize(EventArgs args)
        {
            try
            {
                ...
                RegionTimer.Start();
                TShock.Log.ConsoleInfo("[TUI] TerrariaUI Loaded");
            }
            catch (Exception e)
            {
                TUI.HandleException(e);
        ...
        private static void OnProjDestroy(BEventArgs.PacketHookArgs<KillProjectile> args)
        {
            var owner = args.Player.Index;
            if (owner is < 0 or > 254)
                return;
            Touch previousTouch = TUI.Session[owner]?.PreviousTouch;
            if (TUI.Session[owner]?.ProjectileID == args.Packet.ProjSlot && previousTouch != null && previousTouch.State != TouchState.End)
            {
        ...
        private static void OnNewProjectile(BEventArgs.PacketHookArgs<SyncProjectile> args)
        {
            try
            {
                ...
                {
                    int tileX = (int)Math.Floor((args.Packet.Position.X + 5) / 16);
                    int tileY = (int)Math.Floor((args.Packet.Position.Y + 5) / 16);

                    if (TUI.Touched(player.Index, new Touch(tileX, tileY, TouchState.Begin,
                            player.HasPermission(TUI.ControlPermission), prefix, 0)))
                        TUI.Session[player.Index].ProjectileID = args.Packet.ProjSlot;
