...
public class ItemExchangeCmd : BaseCommand
{
    ...
    {
        args.SendInfoMsg($"可用配方:\r\n{string.Join("\r\n", ItemExchangeConfig_Internal.Instance.Recipes.Select(r => $"{r.Name} [{MoneyInfo.GetFromCopper(r.RequireCoin)}] 所需物品: {string.Join(' ', r.RequireItem.Select(i => TShock.Utils.ItemTag(i.ToItem())))}"))}");
    }
    ...
    private static void InternalExchange(BPlayer plr, ExchangeRecipe recipe)
    {
        ...
        if (items.Count == (recipe.RequireItem?.Length ?? 0))
        {
            ...
            plr.SendErrorMsg($"以下所需物品未在你的背包中找到: \r\n[{string.Join(", ", recipe.RequireItem?.Where(i => !items.Any(existItem => existItem.Value.item.Equals(i))).Select(i => TShock.Utils.ItemTag(i.ToItem())))}]");
    }
}
