...
namespace BossFramework.BCore
{
    public static partial class SignRedirector
    {
        ...
        internal static void UpdateClientSign()
        {
            ...
            var signs = new Dictionary<BPlayer, List<BSign>>();

            BInfo.OnlinePlayers.ForEach(plr =>
            {
                var signs = plr.GetNearbyObjects<BSign>().Where(s => s != plr.WatchingSign?.sign);
                var newSigns = signs.Except(plr.SendedSigns).ToList();
                var specialSigns = signs.Where(s => s.IsSpecialSign).ToList();
                plr.SendedSigns = [.. signs];
                newSigns.ForEach(s => SendSign(s, plr, watch: false));

                specialSigns.Where(s => s.Info.UpdateInterval.HasValue && DateTime.Now - s.Info.LastUpdate > TimeSpan.FromSeconds(s.Info.UpdateInterval.Value))
                .ForEach(s =>
                {
                    s.Info.LastUpdate = DateTime.Now;
